...
> 另请参阅：[architecture.md](architecture.md)（项目架构）、[performance.md](performance.md)（性能调优）

## 环境准备

### 必需

- **Rust 1.75+**（见 `Cargo.toml` 中的 `rust-version`）
- **Git**
...
- **二进制**: `fqc compress` / `decompress` / `info` / `verify` / `custom args`
- **单元测试**: 全量 / 按名过滤
...
| `test_algo` | 19 | 算法测试 (ID/质量值压缩器, PE 优化器) |
| `test_e2e` | 15 | 端到端压缩/解压往返 |
| `test_dna` | 15 | DNA 工具函数测试 |
| **合计** | **131** | |

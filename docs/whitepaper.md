...
| `qvz` | 固定 8 级最近邻码本后再 SCM（非训练率失真 QVZ） | 有损；重建值为码本中的 Phred |
| `discard` | 占位值 | 质量值不使用时最大压缩 |

无损模式利用相邻位置通常具有相似分数（自相关）的观察结果。Illumina8 模式将 41 级 Phred 量表映射到 8 个箱。`qvz` 使用另一组 8 级重建值做最近邻量化，仍走 SCM，不是论文里训练过的率失真 QVZ。

### 4.4 ID 和辅助数据压缩

读段标识符通常遵循可预测的模式（例如 `@HWI-EAS100:6:1:2:1824#0/1`）。`--id-mode` 选择编码（默认 `tokenize`）：

1. `tokenize`：识别公共前缀/后缀，提取可变数字字段；模式不稳定则该块回退 exact
2. `exact`：始终按原文字符串 + Zstd 存储
3. `discard`：解码时写占位符 ID
...
| **fqc (Archive)** | **2.39&times;**（微型测试集实测） | **是** | **否** | **Rust** |
| Spring | 5.0&ndash;15&times;* | 否 | 是 | C++ |
| CRAM | 4.0&ndash;20&times;* | 是 | 是 | C |

> 表中 gzip/zstd/DSRC 2/Leon/Spring/CRAM 的压缩比为文献或经验区间，未在本项目环境中复测；fqc 数值来自本报告 7.1 节的微型测试集实测。大型数据集上的相对性能尚未验证。

在无参考短读段数据上，fqc 的 ABC 算法配合重排具备接近 DSRC 2 和 Leon 的潜力（尚未在大型数据集验证），同时提供它们所缺乏的随机访问能力。
